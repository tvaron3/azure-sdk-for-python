...
async def create_items(client_id):
    ...
    logger.addHandler(logging.StreamHandler(sys.stdout))
    async with AsyncClient(COSMOS_URI2, COSMOS_KEY2, preferred_locations=preferred_regions_2,
                           enable_diagnostics_logging=True, logger=logger, user_agent="Concurrent-Write-Tomas" + str(client_id) + datetime.now().strftime("%Y%m%d-%H%M%S")) as client:
